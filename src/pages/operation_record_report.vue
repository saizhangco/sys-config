...
<script setup>
  ...
  const headers = ref([
    ...
    { title: '結果', key: 'result', align: 'end', sortable: false },
    { title: '詳細資料', key: 'content', align: 'end', sortable: false },
  ])
  ...
  function loadItems (page_num = 1, page_size = 5) {
    loading.value = true
    page_num = page_num - 1
    axios.get('/api/statement/operation_record_report?page=' + page_num + '&size=' + page_size).then(
      response => {
